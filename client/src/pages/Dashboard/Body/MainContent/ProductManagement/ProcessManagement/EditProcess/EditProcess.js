...
function EditProcess({ showModal, setShowModal, data }) {
  ...
  useEffect(() => {
    ...
    let findStep = step_items.filter((step) => step?.value === data?.step);

    setSelectProc([
      {
        id: findStep[0]?.id,
        value: findStep[0]?.value,
      },
    ]);
  }, [data]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!signal || selectProc[0].id === "") {
      ...
      dispatch(updateProcess(procData, data._id));

      dispatch(getProcByProds(data.productId));

      toast("Tùy chỉnh quy trình thành công", {
        position: "bottom-right",
        autoClose: 1000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  return (
    <form autoComplete="off" noValidate onSubmit={handleSubmit}>
      <Modal show={showModal} setShow={setShowModal}>
        <ModalHeader>
          <h2 className="pt-2 mb-0 h2-text">Tùy chỉnh quy trình</h2>
        </ModalHeader>
        <ModalBody>
          <div className="mb-4">
            <label
